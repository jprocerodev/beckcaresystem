...
if (!$result || mysqli_num_rows($result) == 0) {
    ...
    // Calculate total loyalty points from all appointments
    while ($row = mysqli_fetch_assoc($result)) {
        $totalLoyaltyPoints += $row['loyalty_points'];
    }

    // Reset the pointer of the result set to fetch data again for display
    ...
    <section>
        <div class="container table-container">
            ...
            <?php else: ?>
                ...
                <?php
// Fetch rewards from tblrewards where the points_required is less than or equal to the user's points
$rewardsQuery = "SELECT * FROM tblrewards WHERE points_required <= $totalLoyaltyPoints";
$rewardsResult = mysqli_query($con, $rewardsQuery);
?>


<?php
// Fetch rewards and check if the user has claimed them
$rewardsQuery = "
    SELECT r.*, 
           IF(c.claim_id IS NOT NULL, 1, 0) AS already_claimed
    FROM tblrewards r
    LEFT JOIN tblclaimed_rewards c ON r.reward_id = c.reward_id AND c.user_id = $userID
    WHERE r.points_required <= $totalLoyaltyPoints
";
$rewardsResult = mysqli_query($con, $rewardsQuery);
?>
...
<div id="rewardsModal" class="modal" style="display:none;">
    <div class="modal-content">
        ...
        <table class="rewards-table">
            ...
            <tbody>
                <?php
                if (mysqli_num_rows($rewardsResult) > 0) {
                    // Populate the rewards table with data from tblrewards
                    while ($reward = mysqli_fetch_assoc($rewardsResult)) {
                        echo "<tr>";
                        echo "<td><span class='points-highlight'>{$reward['points_required']} Points</span></td>";
                        echo "<td>{$reward['rewardname']}</td>";
                        echo "<td>";
                        if ($reward['already_claimed']) {
                            // Show claimed status if user already claimed the reward
                            echo "<span class='claimed-status'>Claimed</span>";
                        } else {
                            // Show claim button if user has enough points and hasn't claimed it
                            echo "<button class='btn btn-success claim-btn' data-reward-id='{$reward['reward_id']}' data-points='{$reward['points_required']}'>Claim</button>";
                        }
                        echo "</td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='3' class='no-data'>No rewards available</td></tr>";
                }
                ?>
            </tbody>
        </table>
...
document.querySelectorAll('.claim-btn').forEach(button => {
    button.addEventListener('click', function() {
        ...
        if (<?php echo $totalLoyaltyPoints; ?> >= requiredPoints) {
            // AJAX request to claim reward
            $.ajax({
                url: './claim_reward.php',
                type: 'POST',
                data: { reward_id: rewardID, user_id: <?php echo $userID; ?> },
                success: function(response) {
                    const data = JSON.parse(response);
                    if (data.success) {
                        alert('Reward claimed! Your code is: ' + data.reward_code);
                        location.reload(); // Refresh to update points if needed
                    } else {
